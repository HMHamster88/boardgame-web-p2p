...
export default ({ init, subscribe, announce }) => {
  ...
  return (config, roomId, onJoinError) => {
    ...
    const rootTopicP = rootTopicPlaintext //sha1(rootTopicPlaintext)
    const selfTopicP = topicPath(rootTopicPlaintext, selfId) //sha1(topicPath(rootTopicPlaintext, selfId))
    const key = appId + "/" + roomId // genKey(config.password || '', appId, roomId)

    const withKey = f => async signal => ({
      type: signal.type,
      sdp: signal.sdp // await f(key, signal.sdp)
    })
    ...
    const handleMessage = relayId => async (topic, msg, signalPeer) => {
      ...
      if (peerId && !offer && !answer) {
        ...
        const [[{ peer, offer }], topic] = await all([
          getOffers(1),
          topicPath(rootTopicPlaintext, peerId)//sha1(topicPath(rootTopicPlaintext, peerId))
        ])
    ...
    all([rootTopicP, selfTopicP]).then(([rootTopic, selfTopic]) => {
      const queueAnnounce = async (relay, i) => {
        const ms = await announce(relay, rootTopic, selfTopic, selfId)
